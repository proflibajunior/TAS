...
public class PedidoVendaDialog extends GenericDialogImpl {
     ...
     * @param readOnly
     */
    public PedidoVendaDialog(java.awt.Frame parent, JSONObject pedido, Boolean readOnly) {
        super(parent, true);
        ...
        if (this.readOnly) {
            plEsq.setPreferredSize(new java.awt.Dimension(5, 5));
            btAddRegistro.setVisible(false);
            btDelRegistro.setVisible(false);  
            cancelar.setVisible(false);
            salvar.setLabel("Ok");
        }
        
        tfCodigo.setEnabled(!this.readOnly);
        tfData.setEnabled(!this.readOnly);
        cbCliente.setEnabled(!this.readOnly);
        grid.setEnabled(!this.readOnly);
    }
    ...
    private void initComponents() {
        ...
        titulo.setText("Pedido de Venda");
    ...
    private void loadclientes() {
        ...
        try {
            ...
            try {
                Object[] clientes = BackendService.findAll("clientes");
                
                cbCliente.removeAllItems();
                cbCliente.addItem(null);
                
                for (Object object : clientes) {
                    JSONObject vo = (JSONObject) object;
                    cbCliente.addItem(vo);
                }
                ...
                dlgError.showError(TasDialogError.DANGER_ERROR, 
                                   "<html>Não foi possível carregar os clientes. Clique em \"Detalhes\" Para visualizar o problema.</html>", 
                                   e.getMessage());
    ...
    private void setPedido(JSONObject pedido) {
        this.pedido = pedido;
        
        //Carrega a lista de todos os itens para montar o grid
        if (this.pedido.get("itens") != null) {
        ...
        //Outros campos
        tfCodigo.setText(this.pedido.getString("codigo"));
        cbCliente.getModel().setSelectedItem(pedido.get("cliente"));
                
        //Data
        if (this.pedido.get("dtpedido") != null) {
            try {
                Date dtpedido = new SimpleDateFormat("yyyy-MM-dd").parse(this.pedido.getString("dtpedido"));
                SimpleDateFormat df = new SimpleDateFormat("ddMMyyyy");
                tfData.setText(df.format(dtpedido));
            } catch (ParseException ex) {
                Logger.getLogger(PedidoVendaDialog.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }
    ...
    public void salvar() {
        ...
        try {
            ...
            try {
                //Codigo
                pedido.put("codigo", tfCodigo.getText());
                
                //Data
                Date dtpedido = new SimpleDateFormat("dd/MM/yyyy").parse(tfData.getText());
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                pedido.put("dtpedido", sdf.format(dtpedido));
                
                //Cliente
                JSONObject cliente = (JSONObject) cbCliente.getModel().getSelectedItem();
                ...
                BackendService.save("pedidos", this.pedido);               
                this.mainframe.setStatus("Pronto!");
            } catch (Exception e) {
                this.fail = true;
    ...
    public void validarDados() throws NotValidateException {
        ...
        if (cbCliente.getModel().getSelectedItem() == null) {
            this.fieldsEmpty.add("O campo \"Cliente\" deve ser preenchido.");
        }
        if (grid.getRowCount() == 0) {
            this.fieldsEmpty.add("É preciso adicionar produtos à pedido.");
        }
    ...
    private void adicionarItem() {
        ...
        ItemPedidoDialog dialog = new ItemPedidoDialog(mainframe);
